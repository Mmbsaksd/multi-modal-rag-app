...
from doc_parser.api.dependencies import get_store, get_azureopenai_client
...
async def health()-> HealthResponse:
    ...
    store = get_store()
    client: AsyncAzureOpenAI = get_azureopenai_client()
    ...
    try:
        ...
        azureopenai_status = "ok"
    except Exception as exc:
        logger.warning("OpenAI health check failed: {}", exc)
        openai_status = f"error: {exc}"

    overall = "ok" if qdrant_status == "ok" and azureopenai_status == "ok" else "degraded"
